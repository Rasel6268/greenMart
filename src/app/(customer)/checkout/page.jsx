...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    const fetchUpozilasByDistrict = async () => {
      ...
      try {
        ...
        setUpozilas([]);
        toast.error("Failed to load upozilas");
      }
    };
  ...
  const initiateSSLCommerzPayment = async (orderData) => {
    ...
    try {
      const paymentData = {
        ...
        ship_name: `${orderData.firstName} ${orderData.lastName}`,
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (data.paymentMethod === "sslCommerce") {
        await initiateSSLCommerzPayment(data);
      } else {
        await createCODOrder(data);
      }
    } catch (error) {
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div
                      ...
                      onClick={() => handlePaymentMethodChange("sslCommerce")}
                    >
                      <div className="flex items-center gap-4">
                        <input
                          type="radio"
