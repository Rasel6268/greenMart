...
const AddBrand = () => {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/brands`, data);
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600 text-sm">Create a new product brand</p>
          </div>
        </div>
        <div className="flex space-x-3">
          ...
          >
            <Save size={18} />
            <span>{isSubmitting || loading ? 'Saving...' : 'Save Brand'}</span>
          </button>
        </div>
      </div>
